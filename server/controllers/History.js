...
const historycontroller = async (req, res) => {
    ...
    // console.log(`dataa : `, historydata)
    const existing = history.find({ historydata })
    if (existing) {
        // console.log("exists")
        await history.deleteMany({ viewer: historydata.viewer, videoid: historydata.videoid })
    }
...
module.exports = { getallhistorycontroller, deletehistory, historycontroller }